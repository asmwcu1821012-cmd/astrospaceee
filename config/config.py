...
class BaseConfig:
    ...
    num_workers: int = 0  # 0 for CPU training
    ...
    precision: str = '32'  # Changed to string format for PyTorch Lightning
    gradient_clip_val: float = 1.0
    max_epochs: int = 100
    early_stopping_patience: int = 10
    
    # GPU optimization configurations
    accelerator: str = 'cpu'  # Hardware accelerator to use. 'gpu' for NVIDIA GPUs, 'cpu' for CPU training.
...
class AstroClassifierConfig(BaseConfig):
    ...
    precision: str = '32'  # Keep mixed precision for efficiency
